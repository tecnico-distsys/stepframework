...
public class BookFlightDialog extends DialogBox {
	...
	public BookFlightDialog(final AirlineManagerAsync server, MultiWordSuggestOracle airports) {
		super(true, true);
		...
		sendButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				server.bookFlight(from, to, passport.getText(), name.getText(),
						new AsyncCallback<ReservationVoucher>() {
					
					public void onFailure(Throwable caught) {
						if (caught instanceof NoAvailableFlightsException) {
					...
					public void onSuccess(ReservationVoucher result) {
						BookFlightDialog.this.hide();
						message.setHTML("Reservation #" + result.getReservationCode()
								+ "<br/>Flight #" + result.getFlightNumber() + "<br/>Departure: "
								+ DateTimeFormat.getMediumDateTimeFormat().format(result.getFlightDate()));
						message.removeStyleName("errorMessage");
						responseWindow.center();
					}
				});
