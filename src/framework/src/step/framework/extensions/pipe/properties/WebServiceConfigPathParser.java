...
class WebServiceConfigPathParser implements ConfigPathParser {
    ...
    /** Character that separates the service, port names in a web service configuration path */
    ...
    public List<String> parseConfigPath(String configPath) throws IllegalArgumentException
    {
        // check config path
        if(configPath == null)
        	throw new IllegalArgumentException("Web service config path can't be null");

        // trim leading and trailing whitespace
        configPath = configPath.trim();

        // empty specifier is default config
        if(configPath.length() == 0) // return empty list to store as default config
            return new ArrayList<String>(0);

        // check namespace specifier beginning and end
        final int idxBegin = configPath.indexOf(BEGIN_NAMESPACE_SPECIFIER);
        if(idxBegin == -1)
            throw new IllegalArgumentException("Begin namespace character " + BEGIN_NAMESPACE_SPECIFIER +
                                               " not found in web service config path " + configPath);
        
        final int idxEnd = configPath.indexOf(END_NAMESPACE_SPECIFIER);
        if(idxEnd == -1)
            throw new IllegalArgumentException("End namespace character " + END_NAMESPACE_SPECIFIER +
                                               " not found in web service config path " + configPath);
        
        if(idxEnd < idxBegin)
            throw new IllegalArgumentException("Begin namespace character " + BEGIN_NAMESPACE_SPECIFIER +
                                               " must appear before end namespace character " + END_NAMESPACE_SPECIFIER +
                                               " in web service config path " + configPath);
        ...
        if(namespace.length() == 0)
            throw new IllegalArgumentException("Namespace can't be empty in web service config path " + configPath);
        ...
        if(configPath.length() > (namespace.length() + NAMESPACE_SPECIFIER_LENGTH))
        {
            ...
            for(int i = 0; i < splitResult.length; i++)
            {
                ...
                if(splitResult[i].length() == 0)
                    throw new IllegalArgumentException("Web service config path " + configPath + " can't contain empty items");
                
                // check if maximum items have been reached
                if(i > MAX_WEB_SERVICE_CONFIG_PATH_ITEMS)
                    throw new IllegalArgumentException("web service config path " + configPath + " can contain, at most, " +
                    								    MAX_WEB_SERVICE_CONFIG_PATH_ITEMS + " items");
     ...
     */
    public static String composeConfigPath(String namespace, String service, String port)
    {
    	if(namespace == null)
    		throw new IllegalArgumentException("Namespace in config path can't be null");
    	if(service == null)
    		throw new IllegalArgumentException("Service in config path can't be null");
    	if(port == null)
    		throw new IllegalArgumentException("Port in config path can't be null");

        StringBuilder sb = new StringBuilder(namespace.length() + service.length() + port.length() +
        									 NAMESPACE_SPECIFIER_LENGTH + WEB_SERVICE_CONFIG_PATH_ITEM_SEPARATOR.length());
