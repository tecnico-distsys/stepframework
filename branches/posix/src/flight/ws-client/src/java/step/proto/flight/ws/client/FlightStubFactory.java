...
import step.proto.flight.wsdl.FlightService;
 ...
 * Flight stub factory
 *
 * Besides the base getService() and getPort() factory methods
 * it also supports getPortUsingConfig() that reads the endpoint address
 * from a configuration file. See ENDPOINT_ADDRESS_CONFIG_PARAMETER_NAME
 * for property name.
 *
 * It could also be extended with a getPortUsingRegistry() method
 * to look-up the server location in a Web Services Registry.
 *
 */
public class FlightStubFactory extends StubFactory<FlightService,FlightPortType> {
    ...
    //
    public final static String ENDPOINT_ADDRESS_CONFIG_PARAMETER_NAME = "step.proto.flight.ws.EndpointAddress";

    public FlightPortType getPortUsingConfig() throws StubFactoryException {
        Config config = Config.getInstance();
        String endpointAddress = config.getInitParameter(ENDPOINT_ADDRESS_CONFIG_PARAMETER_NAME);
