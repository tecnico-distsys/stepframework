...
import step.proto.flight.ws.client.FlightStubFactory;
...
public class CreateFlightReservationService extends FlightBaseService<ReservationVoucher> {
    ...
    protected ReservationVoucher action() throws FlightDomainException {
        try {
            ...
            FlightPortType port = FlightStubFactory.getInstance().getPortUsingConfig();
            ...
            ReservationVoucher voucher = port.createReservation(this.origin, this.destination, passenger);

            log.debug("Reservation code: " + voucher.getReservationCode());
            log.debug("Reservation flight number: " + voucher.getFlightNumber());

            return voucher;
        } catch (FlightFault_Exception e) {
            // remote domain exception
            log.error(e);
            FlightDomainException ex = ExceptionParser.parse(e.getFaultInfo().getFaultType(), e.getMessage());
            throw ex;
